...
public:
  uint8_t _row;
  uint8_t _column;
  SudokuCoords(uint8_t row = 0, uint8_t column = 0)
...
private:
  /** Sudoku side length can only be squares of natural numbers, hence _rootSize
   * ans _size*/
  uint16_t _rootSize;
  uint16_t _size;
...
public:
  ...
  bool check(uint16_t row, uint16_t col, uint8_t number,SudokuMeta meta = SudokuMeta::Filled);

  /** Check if sudoku is filled*/
  bool isComplete() const;

  /** Applies move with no regard for correctness*/
  void applyMove(SudokuCoords & pos, uint8_t newCell);
  ...
  static uint8_t constructCell(uint8_t number,
                               SudokuMeta meta = SudokuMeta::Filled);
